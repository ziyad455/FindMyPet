...
export const Header: React.FC = () => {
  ...
  const navLinkClass = (path: string) =>
    `relative px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${isActive(path)
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center md:gap-1">
            ...
            <div className="relative flex items-center">
              <div className="absolute left-3 pointer-events-none text-gray-400 group-hover:text-primary-500 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <select
                ...
                className="pl-9 pr-6 py-2 rounded-xl text-sm font-medium text-gray-600 bg-white border border-gray-100 hover:border-primary-200 hover:text-primary-600 focus:outline-none focus:ring-4 focus:ring-primary-100 transition-all duration-200 appearance-none cursor-pointer"
              >
          ...
          >
            <div className="flex flex-col gap-1">
              <Link
                ...
                className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive('/') ? 'text-primary-600 bg-primary-50' : 'text-gray-700 hover:bg-gray-50'
                  }`}
              >
                {t('header.home')}
              </Link>
                ...
                <>
                  {isAdmin ? (
                    ...
                    <Link
                      to="/admin"
                      onClick={() => setMobileMenuOpen(false)}
                      className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive('/admin') ? 'text-primary-600 bg-primary-50' : 'text-gray-700 hover:bg-gray-50'
                        }`}
                    >
                      {t('header.admin')}
                    </Link>
                    ...
                    <>
                      <Link
                        to="/dashboard"
                        onClick={() => setMobileMenuOpen(false)}
                        className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive('/dashboard') ? 'text-primary-600 bg-primary-50' : 'text-gray-700 hover:bg-gray-50'
                          }`}
                      >
                        {t('header.dashboard')}
                      </Link>
                      <Link
                        to="/submit"
                        onClick={() => setMobileMenuOpen(false)}
                        className={`px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive('/submit') ? 'text-primary-600 bg-primary-50' : 'text-gray-700 hover:bg-gray-50'
                          }`}
                      >
                        {t('header.addPet')}
                      </Link>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['ar', 'fr', 'en'] as const).map((lang) => (
                    <button
                      ...
                      className={`py-2 rounded-lg text-sm font-medium transition-all ${language === lang
